...
There are 26 who are not in favor of impeachment, and 209 that are.
...
impeach%>%
  group_by(for_impeachment) %>%
  summarize(mean_pct_bachelors = mean(pct_bachelors, na.rm = TRUE))
...
impeach%>%
  group_by(for_impeachment) %>%
  summarize(mean_trump_percent = mean(trump_percent, na.rm = TRUE))
...
impeach%>%
  filter(party == "D") %>% 
  group_by(for_impeachment, median_age) %>%  
  summarize(mean_median_size = mean(median_age, na.rm = TRUE))

In this code I compared for Democratic members who were for or against impeachment and their median age.
